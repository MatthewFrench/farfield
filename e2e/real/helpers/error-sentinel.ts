...
export class ErrorSentinel {
  ...
  public async writeSummary(): Promise<string> {
    ...
    const outputDirectory = path.join(process.cwd(), ".runtime", "e2e-sentinel");
    ...
    await this.testInfo.attach("e2e-sentinel-summary", {
      body: Buffer.from(JSON.stringify(summary, null, 2), "utf8"),
      contentType: "application/json"
    });

    process.stdout.write(
      `[e2e-sentinel] ${this.scenarioId}: debug=${String(this.newErrorEvents.length)} api=${String(this.failedApiResponses.length)} warn=${String(this.consoleWarnings.length)} err=${String(this.consoleErrors.length)} page=${String(this.pageErrors.length)} banner=${String(this.bannerEvents.length)} timeout=${String(this.loadingTimeoutBreaches.length)}\n`
    );
    process.stdout.write(`[e2e-sentinel] summary ${summaryPath}\n`);
