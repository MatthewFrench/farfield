...
const DebugClientErrorsSessionLogWithTrailingSlashRoutePath =
...
const DebugClientErrorsMissingIdentifierWithTrailingSlashRoutePath =
  "/api/debug/client-errors/error_missing/";
...
async function postReplayRequest(
  ...
  return fetch(integrationEnvironment.buildApiRouteUrl(DebugReplayRoutePath), {
    method: "POST",
    headers: {
      ...authHeaders,
...
async function readHistoryEntryIdentifierForReplay(
  ...
  if (!replayHistoryEntry) {
    throw new Error(
      "Expected at least one debug history entry for replay route integration checks",
    );
  }
...
describe("server route integration debug routes", () => {
  ...
  it("supports debug client-error contracts", async () => {
    ...
    const createResponse = await fetch(
      integrationEnvironment.buildApiRouteUrl(DebugClientErrorsRoutePath),
      {
        method: "POST",
        headers: {
          ...authHeaders,
    ...
    const listAfterClearResponse = await fetch(
      buildDebugClientErrorListRouteUrl(integrationEnvironment),
      {
        headers: authHeaders,
      },
    );
    expect(listAfterClearResponse.status).toBe(200);
    const listedAfterClear = DebugErrorListEnvelopeSchema.parse(
      await listAfterClearResponse.json(),
    );
  ...
  it("returns deterministic 400 contracts for invalid debug identifier segments", async () => {
    ...
    await expectApiErrorResponse(malformedTraceResponse, 400, InvalidTraceIdentifierErrorMessage);
  ...
  it("supports debug observability snapshot contracts", async () => {
    ...
    const response = await fetch(
      integrationEnvironment.buildApiRouteUrl(DebugObservabilityRoutePath),
      {
        headers: authHeaders,
      },
    );
    ...
    expect(
      payload.snapshot.routing.threadAdapterResolver.unregisteredDiscoveryAttemptCount,
    ).toBeGreaterThanOrEqual(0);
    expect(
      payload.snapshot.performance.requestRouting.requestLifecycleEvents.length,
  ...
  it("returns deterministic 404 contracts for unowned and near-match debug routes", async () => {
    ...
    const debugPrefixResponse = await fetch(
      integrationEnvironment.buildApiRouteUrl(DebugRoutePathPrefix),
      {
        headers: authHeaders,
      },
    );
    ...
    const trailingSlashSessionLogResponse = await fetch(
      integrationEnvironment.buildApiRouteUrl(
        DebugClientErrorsSessionLogWithTrailingSlashRoutePath,
      ),
      {
        headers: authHeaders,
      },
    ...
    const trailingSlashClientErrorIdentifierResponse = await fetch(
      integrationEnvironment.buildApiRouteUrl(
        DebugClientErrorsMissingIdentifierWithTrailingSlashRoutePath,
      ),
      {
        headers: authHeaders,
      },
    ...
    await expectApiErrorResponse(
      trailingSlashClientErrorIdentifierResponse,
      404,
