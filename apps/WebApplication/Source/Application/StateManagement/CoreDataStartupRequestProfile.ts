...
const STARTUP_REQUEST_PROFILE_SCHEMA_ERROR_PREFIX =
  "Startup request profile schema validation failed";
...
const StartupRequestProfileEntrySchema = z
  .object({
    actionName: z.string(),
...
export const STARTUP_DEFERRED_THREADS_REVALIDATE_OPERATION =
  "startup-deferred.threads.active.revalidate";
...
function readStartupRequestProfileSchemaError(entryIndex: number, error: z.ZodError): Error {
  ...
  const profileSchemaErrorMessage = `${STARTUP_REQUEST_PROFILE_SCHEMA_ERROR_PREFIX} at entry index ${String(entryIndex)}: ${issueMessages}`;
...
function initializeStartupRequestProfileOwnerState(
  ...
  for (
    let profileEntryIndex = 0;
    profileEntryIndex < startupRequestProfileEntries.length;
    profileEntryIndex += 1
  ) {
...
const STARTUP_REQUEST_PROFILE_OWNER_STATE =
  initializeStartupRequestProfileOwnerState(STARTUP_REQUEST_PROFILE);
...
export function readStartupRequestDescription(actionName: string): string {
  return (
    STARTUP_REQUEST_PROFILE_OWNER_STATE.requestDescriptionByActionName.get(actionName) ?? actionName
  );
}
