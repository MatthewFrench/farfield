...
const ERROR_BANNER_ACTION_IDENTIFIER_CAPTURE_PATTERN = /\baction(?:\s*id|Id)\b[ =:]+([a-z0-9._-]+)/i;
const ERROR_BANNER_REQUEST_IDENTIFIER_CAPTURE_PATTERN = /\brequest(?:\s*id|Id)\b[ =:]+([a-z0-9._-]+)/i;
...
const ErrorBannerIdentifierSchema = z
  .string()
  .trim()
  .min(1)
  .regex(ERROR_BANNER_IDENTIFIER_PATTERN);
