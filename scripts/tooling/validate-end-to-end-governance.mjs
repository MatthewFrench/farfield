...
const matrixPath = path.join(process.cwd(), "end-to-end", "real", "coverage-matrix.md");

function fail(message) {
  process.stderr.write(`[end-to-end-governance] ${message}\n`);
  process.exit(1);
}
...
if (tableRows.length <= 1) {
  process.stdout.write("[end-to-end-governance] Open gaps table has no active entries\n");
  process.exit(0);
}
...
process.stdout.write(
  `[end-to-end-governance] Open gap checks passed (${String(activeGaps.length)} active gap entries)\n`
);
