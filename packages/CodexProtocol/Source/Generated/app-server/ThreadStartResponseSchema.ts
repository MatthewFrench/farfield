...
export const ThreadStartResponseSchema = z.object({
  ...
  reasoningEffort: z
    ...
    .optional(),
  sandbox: z.any().superRefine((x, ctx) => {
    const schemas = [
      ...
      z.object({
        access: z
          ...
          .superRefine((x, ctx) => {
            const schemas = [
              z.object({
                includePlatformDefaults: z.boolean().default(true),
                readableRoots: z
                  .array(
                    z
                      .string()
      ...
      z.object({
        networkAccess: z.enum(["restricted", "enabled"]).default("restricted"),
  ...
  thread: z.object({
    cliVersion: z.string().describe("Version of the CLI that created the thread."),
    createdAt: z
      .number()
      .int()
      .describe("Unix timestamp (in seconds) when the thread was created."),
    cwd: z.string().describe("Working directory captured for the thread."),
    gitInfo: z
      .union([
        z.object({
          branch: z.union([z.string(), z.null()]).optional(),
          originUrl: z.union([z.string(), z.null()]).optional(),
          sha: z.union([z.string(), z.null()]).optional(),
        }),
        z.null(),
      ])
      .describe("Optional Git metadata captured when the thread was created.")
      .optional(),
    id: z.string(),
    modelProvider: z
      .string()
      .describe("Model provider used for this thread (for example, 'openai')."),
    path: z
      .union([
        z.string().describe("[UNSTABLE] Path to the thread on disk."),
        z.null().describe("[UNSTABLE] Path to the thread on disk."),
      ])
      .describe("[UNSTABLE] Path to the thread on disk.")
      .optional(),
    preview: z.string().describe("Usually the first user message in the thread, if available."),
    source: z
      .any()
      .superRefine((x, ctx) => {
        const schemas = [
          ...
          z
            .object({
              subAgent: z.any().superRefine((x, ctx) => {
                const schemas = [
      ...
      .describe("Origin of the thread (CLI, VSCode, codex exec, codex app-server, etc.)."),
    turns: z
      .array(
        z.object({
          error: z
            .union([
              z.object({
                additionalDetails: z.union([z.string(), z.null()]).default(null),
                codexErrorInfo: z
                  .union([
                    z
                      .any()
                      .superRefine((x, ctx) => {
                        const schemas = [
                          ...
                          z
                            ...
                            .strict()
                            .describe("Failed to connect to the response SSE stream."),
                          z
                            .object({
                          ...
                          z
                            ...
                            .strict()
                            .describe("Reached the retry limit for responses."),
                        ];
                        const errors = schemas.reduce<z.ZodError[]>(
                          (errors, schema) =>
                            ((result) => (result.error ? [...errors, result.error] : errors))(
            ...
            .describe("Only populated when the Turn's status is failed.")
            .optional(),
          id: z.string(),
          items: z
            .array(
                ...
                const schemas = [
                  ...
                  z.object({
                    aggregatedOutput: z
                      .union([
                        z
                          .string()
                          .describe("The command's output, aggregated from stdout and stderr."),
                        z
                          .null()
                          .describe("The command's output, aggregated from stdout and stderr."),
                      ])
                    ...
                    cwd: z.string().describe("The command's working directory."),
                    durationMs: z
                      .union([
                        z
                          .number()
                          .int()
                          .describe("The duration of the command execution in milliseconds."),
                        z.null().describe("The duration of the command execution in milliseconds."),
                      ])
                      .describe("The duration of the command execution in milliseconds.")
                      .optional(),
                    exitCode: z
                      .union([
                        z.number().int().describe("The command's exit code."),
                        z.null().describe("The command's exit code."),
                      ])
                    ...
                    processId: z
                      .union([
                        z
                          .string()
                          .describe("Identifier for the underlying PTY process (when available)."),
                        z
                          .null()
                          .describe("Identifier for the underlying PTY process (when available)."),
                      ])
                    ...
                    durationMs: z
                      .union([
                        z
                          .number()
                          .int()
                          .describe("The duration of the MCP tool call in milliseconds."),
                        z.null().describe("The duration of the MCP tool call in milliseconds."),
                      ])
                      .describe("The duration of the MCP tool call in milliseconds.")
                      .optional(),
                    error: z.union([z.object({ message: z.string() }), z.null()]).optional(),
                    id: z.string(),
                    result: z
                      .union([
                        z.object({
                  ...
                  z.object({
                    agentsStates: z
                      ...
                      .describe("Last known status of the target agents, when available."),
                    id: z.string().describe("Unique identifier for this collab tool call."),
                    prompt: z
                      .union([
                        z
                          .string()
                      ...
                      .describe("Prompt text sent as part of the collab tool call, when available.")
                      .optional(),
                    receiverThreadIds: z
                      .array(z.string())
                      .describe(
                    ...
                    senderThreadId: z
                      .string()
                      .describe("Thread ID of the agent issuing the collab request."),
                    status: z
                      .enum(["inProgress", "completed", "failed"])
                      .describe("Current status of the collab tool call."),
                    tool: z
                      .enum(["spawnAgent", "sendInput", "resumeAgent", "wait", "closeAgent"])
                      .describe("Name of the collab tool that was invoked."),
    ...
    updatedAt: z
      .number()
      .int()
      .describe("Unix timestamp (in seconds) when the thread was last updated."),
  }),
});
