...
| Thread row maintenance actions + identity stability | Trigger row-menu maintenance actions and verify thread row plus runtime-status badge keep stable node identity | `end-to-end/real/scenarios/thread-row-menu-maintenance-actions.spec.ts` (`row-menu compact and clean actions keep row identity stable and show success feedback`) | `MANUAL-THREAD-ROW-MAINTENANCE` | `@farfield` | `2026-03-01` |
