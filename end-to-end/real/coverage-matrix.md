...
| Empty thread creation + loaded-state semantics | Create a thread from the sidebar, keep the new empty thread selected, and show explicit loaded-state/first-message guidance | `end-to-end/real/scenarios/empty-thread-create-and-open.spec.ts` (`sidebar new-thread flow keeps empty thread selected and explains loaded state`) | `MANUAL-EMPTY-THREAD-CREATE` | `@farfield` | `2026-03-12` |
...
| Mobile sidebar freeze profiling | Repeatedly open and close the mobile sidebar, collect freeze windows/long tasks, and correlate them with app-owned operations | `end-to-end/real/scenarios/mobile-sidebar-freeze-profile.spec.ts` (`mobile sidebar freeze profile`) | `MANUAL-MOBILE-SIDEBAR-FREEZE` | `@farfield` | `2026-03-06` |
