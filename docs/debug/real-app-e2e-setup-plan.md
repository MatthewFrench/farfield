# Real App E2E Setup Plan (Detailed)
...
- Frontend Vite dev server runs on `127.0.0.1:4312` (`apps/web/vite.config.ts`).
- Backend server runs on `127.0.0.1:4311` (`apps/server/src/index.ts`).
- Frontend proxies `/api` and `/events` to backend (`apps/web/vite.config.ts`).
- API token injection in dev proxy is restricted to trusted origins (`localhost` defaults, configurable via `VITE_DEV_PROXY_TRUSTED_ORIGINS`) when `API_TOKEN` or `PUSH_API_TOKEN` is set (`apps/web/vite.config.ts`).
...
2. Command surface in root `package.json`
   ...
   - `e2e:real:ui`
   - `e2e:real:debug`
   - `verify:real` (`smoke:app` + `e2e:real:run`)
3. Test structure under `e2e/real/`
   ...
   - `coverage-matrix.md`
   - `README.md`
4. Selector hardening in `apps/web/src/App.tsx`
...
7. Coverage governance
   - Add and maintain `e2e/real/coverage-matrix.md` so every feature area maps to at least one scenario.
...
- `pnpm e2e:real:debug -- --grep "thread-open"`
  - Debug one scenario interactively.
- `pnpm verify:real`
  - `pnpm smoke:app` then `pnpm e2e:real:run`.
...
- Add `verify:real` composite script for one-shot verification.
- Ensure each `e2e:real:*` script is executed via `scripts/with-env.mjs` for env parity with `pnpm dev`.
...
#### `e2e/real/fixtures/real-app.fixture.ts`
...
- Writes scenario signal summary to `.runtime/e2e-sentinel/<scenario>.ndjson` and prints a compact terminal summary.
- Refreshes `.runtime/e2e-sentinel/latest.ndjson` for live monitoring during iterative runs.
...
#### `e2e/real/helpers/error-sentinel.ts`
...
#### `e2e/real/helpers/signal-allowlist.ts`
...
#### `e2e/real/helpers/app-actions.ts`
...
#### `e2e/real/helpers/app-assertions.ts`
...
#### `e2e/real/helpers/diagnostics.ts`
...
Add `e2e/real/coverage-matrix.md` with rows mapping:
...
- Artifact file:
  - write full structured signal report to `.runtime/e2e-sentinel/<scenario>.ndjson`.
  - update `.runtime/e2e-sentinel/latest.ndjson` with the most recent scenario summary for live tailing.
...
- Add `e2e/real/README.md` with run/debug instructions.
- Update root docs to point to the same loop.

### Phase 5: Coverage governance

- Add `e2e/real/coverage-matrix.md`.
...
2. `pnpm e2e:real:run` passes locally with both:
   - empty thread account
   - account with existing threads
3. `pnpm e2e:real:ui` supports fast reruns while `pnpm dev` is active.
...
8. `e2e/real/coverage-matrix.md` exists and is updated for feature-surface changes.
