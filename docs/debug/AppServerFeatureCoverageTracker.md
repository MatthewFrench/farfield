...
1. Keep-recommendation coverage at request-owner layer: `53 / 53` (`100.0%`).
2. Keep-recommendation coverage at transport handshake layer (`initialize`): `1 / 1` (`100.0%`).
3. Keep-recommendation coverage including transport-owned `initialize`: `54 / 54` (`100.0%`).
4. Do-not-adopt avoidance completion: `20 / 20` (`100.0%`).
5. Anti-completion count (methods marked `Do not adopt` that are still used): `0`.
6. Policy completion score (`keep used + do-not-adopt avoided - do-not-adopt used`): `74 / 74` (`100.0%`).
7. Legacy raw usage metric across all upstream request methods (reference only, not completion): `54 / 74` (`73.0%`).
8. Event-surface policy completion score (`keep active + do-not-adopt avoided - do-not-adopt active`): `54 / 54` (`100.0%`); anti-completion events used: `0`.
<!-- APP_SERVER_FEATURE_TRACKER_METRICS_END -->
9. Farfield now captures app-server notification streams and exposes them through thread stream-event reads and dedicated notification-event reads in debug coverage diagnostics.
10. Farfield now handles all upstream app-server server-request methods (`item/commandExecution/requestApproval`, `item/fileChange/requestApproval`, `item/tool/requestUserInput`, `item/tool/call`, `account/chatgptAuthTokens/refresh`, `applyPatchApproval`, `execCommandApproval`) with typed response contracts and user-facing response controls for current product flows.
11. Farfield now exposes pending unresolved app-server server-request snapshots through debug coverage diagnostics (`/api/server-requests/pending`) with strict typed request metadata and payload previews.
12. Farfield now consumes auth-completion notifications (`mcpServer/oauthLogin/completed`, `account/login/completed`) through dedicated debug coverage diagnostics with strict typed event mapping and cursor-safe reads, and projects auth completion outcomes into header runtime warning banners.
13. Farfield now consumes `serverRequest/resolved` notifications through dedicated debug coverage diagnostics with strict typed event mapping and explicit pending-request refresh controls, and projects selected-thread resolved request summaries into header runtime warning banners.
14. Farfield now consumes fuzzy-session notifications (`fuzzyFileSearch/sessionUpdated`, `fuzzyFileSearch/sessionCompleted`) through dedicated debug coverage diagnostics with strict typed session/result mapping and cursor-safe reads.
15. Farfield now consumes `model/rerouted` notifications through dedicated debug coverage diagnostics with strict typed reroute mapping and explicit reason visibility, and projects selected-thread reroutes into a visible header runtime banner.
16. Farfield now consumes warning notifications (`configWarning`, `deprecationNotice`, `windows/worldWritableWarning`) through dedicated debug coverage diagnostics with strict typed warning payload mapping and cursor-safe reads, and projects selected-thread warning summaries into a visible header warning banner.
17. Farfield now consumes thread-lifecycle notifications (`thread/archived`, `thread/unarchived`, `thread/name/updated`, `thread/closed`, `thread/status/changed`) through dedicated debug coverage diagnostics with strict typed lifecycle/state mapping and thread-filtered reads, and projects selected-thread archive/unarchive/close summaries into visible header runtime banners.
18. Farfield now consumes `error` notifications through dedicated debug coverage diagnostics with strict typed turn-error mapping and retry-state visibility, and projects selected-thread error summaries into a visible header runtime banner.
19. Farfield now consumes turn-lifecycle notifications (`turn/started`, `turn/completed`, `turn/plan/updated`, `turn/diff/updated`) through dedicated debug coverage diagnostics with strict typed lifecycle mapping and cursor-safe reads, and projects selected-thread turn start/completion/plan/diff updates into sidebar progress summary chips.
20. Farfield now consumes item-delta notifications (`item/agentMessage/delta`, `item/plan/delta`, `item/reasoning/summaryTextDelta`, `item/reasoning/summaryPartAdded`, `item/reasoning/textDelta`, `item/commandExecution/outputDelta`, `item/commandExecution/terminalInteraction`, `item/fileChange/outputDelta`, `item/mcpToolCall/progress`) through dedicated debug coverage diagnostics with strict typed delta payload mapping and cursor-safe reads.
21. Farfield now consumes item-lifecycle and raw-response notifications (`item/started`, `item/completed`, `rawResponseItem/completed`) through dedicated debug coverage diagnostics with strict typed item identity mapping and cursor-safe reads.
22. Farfield now consumes thread-progress notifications (`thread/started`, `thread/compacted`, `thread/tokenUsage/updated`) through dedicated debug coverage diagnostics with strict typed thread/token mapping and cursor-safe reads, and projects selected-thread progress plus token usage into sidebar runtime summary chips.
23. Farfield now consumes thread-realtime notifications (`thread/realtime/started`, `thread/realtime/itemAdded`, `thread/realtime/outputAudio/delta`, `thread/realtime/error`, `thread/realtime/closed`) through dedicated debug coverage diagnostics with strict typed realtime payload mapping and cursor-safe reads, and projects selected-thread realtime started/closed plus realtime error summaries into visible header runtime banners.
24. Farfield now consumes account/app/sandbox update notifications (`account/updated`, `account/rateLimits/updated`, `app/list/updated`, `windowsSandbox/setupCompleted`) through dedicated debug coverage diagnostics with strict typed update-state mapping and cursor-safe reads, and projects account/rate-limit/app-list updates into sidebar runtime summary chips with capability-gated initial hydration.
25. Farfield runtime composition now projects notification-event reads for `thread/status/changed`, `thread/started`, `thread/compacted`, `turn/started`, `turn/completed`, `turn/plan/updated`, `turn/diff/updated`, `thread/tokenUsage/updated`, `model/rerouted`, `configWarning`, `deprecationNotice`, `windows/worldWritableWarning`, `thread/archived`, `thread/unarchived`, `thread/closed`, `thread/realtime/started`, `thread/realtime/closed`, `thread/realtime/error`, `error`, `mcpServer/oauthLogin/completed`, `account/login/completed`, and `serverRequest/resolved` into product runtime surfaces (thread-row status badges, sidebar progress/token-usage chips, and header reroute/warning/error/realtime/auth/request banners) with strict schema parsing, deterministic selected-thread merges, bounded read observability ownership, and explicit runtime banner policy ownership for dedupe/priority/time-to-live/thread-switch clearing behavior.
...
13. `apps/WebApplication/Source/Features/Debugging/StateManagement/DebugAppServerCoverageItemDeltaNotificationMappers.ts` projects item delta/progress notifications into strict item-delta diagnostics.
14. `apps/WebApplication/Source/Features/Debugging/StateManagement/DebugAppServerCoverageItemLifecycleNotificationMappers.ts` projects `item/started` and `item/completed` notification payloads into strict item-lifecycle diagnostics.
