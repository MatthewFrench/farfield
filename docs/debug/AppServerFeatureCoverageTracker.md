...
Last Updated (UTC): 2026-03-01 15:10:00Z
...
17. Farfield now consumes thread-progress notifications (`thread/started`, `thread/compacted`, `thread/tokenUsage/updated`) through dedicated debug coverage diagnostics with strict typed thread/token mapping and cursor-safe reads, and projects selected-thread token usage into sidebar runtime summary chips.
...
20. Farfield runtime composition now projects notification-event reads for `thread/status/changed`, `thread/tokenUsage/updated`, and `model/rerouted` into product runtime surfaces (thread-row status badges, sidebar token-usage chips, and header reroute banners) with strict schema parsing, deterministic selected-thread merges, and bounded read observability ownership.
