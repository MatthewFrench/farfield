...
20. Farfield runtime composition now projects notification-event reads for `thread/status/changed`, `thread/started`, `thread/compacted`, `turn/started`, `turn/completed`, `thread/tokenUsage/updated`, `model/rerouted`, `configWarning`, `deprecationNotice`, `windows/worldWritableWarning`, `thread/realtime/started`, `thread/realtime/closed`, `thread/realtime/error`, and `error` into product runtime surfaces (thread-row status badges, sidebar progress/token-usage chips, and header reroute/warning/error/realtime banners) with strict schema parsing, deterministic selected-thread merges, and bounded read observability ownership.
